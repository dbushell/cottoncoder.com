...
export const hash = (value: string) => new MurmurHash3(value).result().toString(16);

export const sha256Hash = (value: string): Promise<ArrayBuffer> =>
  crypto.subtle.digest('SHA-256', new TextEncoder().encode(value));

const randomIV = (length: number): Uint8Array => crypto.getRandomValues(new Uint8Array(length));
...
export const encryptText = async (value: string, key: CryptoKey | string): Promise<string> => {
...
export const decryptText = async (value: string, key: CryptoKey | string): Promise<string> => {
